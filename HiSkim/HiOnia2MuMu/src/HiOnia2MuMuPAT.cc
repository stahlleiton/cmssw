...
//6: $\phi -> \phi+\pi$
//7: $\phi -> \phi-\pi/2$
const reco::TrackBase::Point HiOnia2MuMuPAT::rotatePoint(reco::TrackBase::Point PV, reco::TrackBase::Point TrkPoint, int flipJpsi){
  float x=TrkPoint.x(), y=TrkPoint.y(), z=TrkPoint.z();
  float vx=PV.x(), vy=PV.y(), vz=PV.z();

  if(flipJpsi<=4){z = 2*vz - z;}

  switch(flipJpsi) {
  	case 1: case 6:
  	  x = 2*vx - TrkPoint.x(); //change frame of reference to place the origin at the PV, rotate, then come back to (0,0,0) origin
  	  y = 2*vy - TrkPoint.y();
  	  break;
  	case 2: case 5:
  	  x = vx -(TrkPoint.y() - vy);
      ...
      if (myVertex.isValid()) {
		...
		if (resolveAmbiguity_) {
		  ...
		  for(VertexCollection::const_iterator itv = priVtxs->begin(), itvend = priVtxs->end(); itv != itvend; ++itv) {
		    ...
		    if ( itv->isFake() || itv->tracksSize() < 2 || fabs(itv->position().z()) > 25 || itv->position().Rho() > 2 ) continue;
		...
		if(!dimuonSelection_(myCand)) {
			if (DimuonTrk_ || (!doTrimuons_) || flipJpsiDirection_>0 ) continue; //if flipJpsi>0 or we do dimuon+track, then we want this dimuon to be the true Jpsi in the trimuon
		    else {goto TrimuonCand;}
		}
	    ...
	    if(thePVsLabel.module==(std::string)("offlinePrimaryVertices")){
	      ...
	      try {
	        for(reco::Vertex::trackRef_iterator itVtx = theOriginalPV.tracks_begin(); itVtx != theOriginalPV.tracks_end(); itVtx++) if(itVtx->isNonnull()){
		      ...
		      if(track.pt() < 0.5) continue; //reject all rejects from counting if less than 500 MeV
