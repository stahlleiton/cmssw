  ...
  for(View<pat::Muon>::const_iterator it = muons->begin(), itend = muons->end(); it != itend; ++it){
	...
	if (myVertex.isValid()) {
	  ...
	  if( addMuonlessPrimaryVertex_ && thePrimaryV.tracksSize()>2) 
		...
		"This is sure to make sense in the case of pp reconstruction and not PbPb reconstruction; Make sure you know what you are doing" << "\n";
		...
		if (muonLess.size()>1 && muonLess.size() < thePrimaryV.tracksSize())
		  {
		    ...
		    else {edm::LogWarning("HiOnia2MuMuPAT_FailingToRefitMuonLessVtx") << 
			"TransientVertex re-fitted is not valid!! You got still the 'old vertex'" << "\n";}
		  }
		else {edm::LogWarning("HiOnia2MuMuPAT_muonLessSizeORpvTrkSize") << 
