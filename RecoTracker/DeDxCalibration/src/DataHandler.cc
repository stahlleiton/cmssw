...
void DataHandler::readStripProps()
{
  ...
  if(pass != -99)
  {
    sprintf(fileName,"RecoTracker/DeDxCalibration/data/stripProps.par");
    cerr << " stripProps.par.." << endl;
  }
  else
  {
    sprintf(fileName,"RecoTracker/DeDxCalibration/data/stripProps_%s.par",
            tag.c_str());
...
void DataHandler::readGainCorrection()
{
  ...
  {
    sprintf(fileName,"RecoTracker/DeDxCalibration/data/gain_%s.dat",tag.c_str());
    cerr << " gain_" << tag << ".dat"; 
  }
        ...
        hit!= track.stripHits.rend(); hit++)
  {
    ...
    if(hit->forEloss && hit->meas <= int(fabs(hit->pred)) + 2 + 2)
    {
      ...
      if(pass < 2 || gain[m.chipId] > 0) 
      {
        ...
        // ADC -> e- -> MeV
        m.energy      =      result.first   * sistrip::MeVperADCStrip;
        m.energySigma = sqrt(result.second) * sistrip::MeVperADCStrip;
                           ...
                           int cha, double bg)
{
                                    ...
                                    m!= meas.end(); m++)
  {
    ...
    if(hits[m->chipId].size() < 1000) // MAX
