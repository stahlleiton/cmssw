...
def candidateBtaggingMiniAOD(process, isMC = True, jetPtMin = 15, jetR = 0.4, jetCorrLevels = ['L2Relative', 'L3Absolute'], doFlow = False):
    ...
    process.svTask = cms.Task(process.svTask, process.inclusiveCandidateNegativeVertexFinder, process.candidateNegativeVertexMerger, process.candidateNegativeVertexArbitrator, process.inclusiveCandidateNegativeSecondaryVertices)
    ...
    getattr(process,f'pfUnifiedParticleTransformerAK4JetTagsAK{jL}DeepFlavour').model_path = 'HeavyIonsAnalysis/Configuration/data/'+(f'PbPb_AK3_2024_v6.onnx' if jetR==0.3 else f'UParTAK{R}_PbPb_2024.onnx')
    getattr(process,f'pfUnifiedParticleTransformerAK4TagInfosAK{jL}DeepFlavour').sort_cand_by_pt = True
    getattr(process,f'pfUnifiedParticleTransformerAK4TagInfosAK{jL}DeepFlavour').fix_lt_sorting = True

    getattr(process,f'pfImpactParameterTagInfosAK{jL}DeepFlavour').maxDeltaR = jetR
    for taginfo in [f"pfDeepFlavourTagInfosAK{jL}DeepFlavour", f"pfParticleTransformerAK4TagInfosAK{jL}DeepFlavour", f"pfUnifiedParticleTransformerAK4TagInfosAK{jL}DeepFlavour"]:
