...
HLTMuTree::HLTMuTree(const edm::ParameterSet& iConfig)
{
  ...
  tokRecoMu = consumes< edm::View<pat::Muon> >(iConfig.getParameter<edm::InputTag>("muons"));
  ...
  tokGenPtl = consumes< reco::GenParticleCollection >(iConfig.getParameter<edm::InputTag>("genparticle"));
...
HLTMuTree::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (doGen) {
    ...
    iEvent.getByToken(tokGenPtl,genColl);
    if (genColl.isValid()) {
...
*/
  ...
  if (doReco) {
    ...
    iEvent.getByToken(tokVtx,vertex);
    if(vertex->size() > 0){
      vx = vertex->begin()->x();
      vy = vertex->begin()->y();
      vz = vertex->begin()->z();
    ...
    iEvent.getByToken(tokRecoMu,muons);

    int nGlb = 0;
    int nSta = 0;

    for (unsigned int i=0; i<muons->size(); i++) {
      edm::RefToBase<pat::Muon> muCand(muons,i);
    ...
    edm::Handle< edm::View<pat::Muon> > muons2;
    iEvent.getByToken(tokRecoMu,muons2);
	...
	if (muCand->isGlobalMuon() && muCand->isTrackerMuon() && fabs(muCand->combinedMuon()->eta()) < 2.4) {
	  for (unsigned int j=i+1; j<muons->size(); j++){
	    edm::RefToBase<pat::Muon> muCand2(muons,j);
