...
config.Data.outLFNDirBase = '\''/store/user/%s/EWQAnalysis2017/MC/NonEmbedded/TTree/%s'\'' % (getUsernameFromSiteDB(), config.General.requestName)
...
for (( i=4; i<5; i++ )); do
    for (( j=1; j<${numTotJobs}+1; j++ )); do
        ...
        logFileName=${stepPaths[$i-1]}/$logPath/step$i\_NonEmbedded\_${workJobs[$j-1]}_${stepTypes[$i-1]}_${stepDates[$i-1]}.log
        rootFileName=step$i\_NonEmbedded\_${workJobs[$j-1]}_${stepTypes[$i-1]}_${stepDates[$i-1]}.root
        crabFileName=${stepPaths[$i-1]}/$crabPath/step$i\_NonEmbedded\_${workJobs[$j-1]}_${stepTypes[$i-1]}_${stepDates[$i-1]}.py
        crabLogFileName=${stepPaths[$i-1]}/$crabPath/step$i\_NonEmbedded\_${workJobs[$j-1]}_${stepTypes[$i-1]}_${stepDates[$i-1]}.log
        cfgFileName=${stepPaths[$i-1]}/$pyPath/step$i\_NonEmbedded\_${workJobs[$j-1]}_${stepTypes[$i-1]}_${stepDates[$i-1]}.py
        if [ $i -gt 1 ]; then
            prevRootFileName=${stepPaths[$i-2]}/$rootPath/step$(expr $i - 1)\_NonEmbedded\_${workJobs[$j-1]}_${stepTypes[$i-2]}_${stepDates[$i-2]}.root
        fi
        ...
        if [ $i -eq 4 ]; then
            ...
            if [ ! -f $crabFileName ]; then
                delete dir "${stepPaths[$i-1]}/$crabPath/crab_projects/crab_NonEmbedded_${workJobs[$j-1]}_${stepTypes[$i-1]}_${stepDates[$i-1]}/"
                echo 'Step 4: Producing '${workJobs[$j-1]}' '${stepTypes[$i-1]}' CRAB config file'
                crab_${stepTypes[$i-1]} NonEmbedded\_${workJobs[$j-1]}\_${stepTypes[$i-1]}\_${stepDates[$i-1]} $cfgFileName ${anaInputPDs[$j-1]} ${anaNumCrabEvts[$j-1]} >> $crabFileName
                cd ${stepPaths[$i-1]}/$crabPath
                crab submit -c $crabFileName --dryrun >& $crabLogFileName
            fi
        fi
