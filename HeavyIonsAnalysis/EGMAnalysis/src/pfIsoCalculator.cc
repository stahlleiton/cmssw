...
void pfIsoCalculator::setVertex(const math::XYZPoint& pv)
{
  vtx_ = pv;
}
...
double pfIsoCalculator::getPfIso(const pat::Photon& photon, int pfId,
                                 double r1, double r2, double threshold,
                                 double jWidth, int footprintRemoval)
{
  bool debug = false;
...
double pfIsoCalculator::getPfIsoSubUE(const pat::Photon& photon, int pfId,
                                      double r1, double r2, double threshold,
                                      double jWidth, int footprintRemoval, bool excludeCone)
{
  double photonEta = photon.eta();
  double photonPhi = photon.phi();
  double totalEt = 0;
                                 ...
                                 double r1, double r2, double threshold)
{
  ...
  for (auto pf = candidatesView->begin(); pf != candidatesView->end(); ++pf) {
    ...
    if (pfId_ == reco::PFCandidate::e) continue;
