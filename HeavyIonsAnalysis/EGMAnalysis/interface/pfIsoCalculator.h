...
class pfIsoCalculator
{
  public:
    ...
    void setCandidatesPacked(const edm::Handle<edm::View< pat::PackedCandidate >> & candidatesPacked) { candidatesView = candidatesPacked; };
    void setVertex(const math::XYZPoint& pv);

    template <class T, class U>
    bool isAssociatedPackedCand(const T& associatedPackedCands, const U& candidate);

    double getPfIso(const pat::Photon& photon, int pfId, double r1=0.4, double r2=0.00, double threshold=0, double jWidth=0.0, int footprintRemoval = 0);
    double getPfIsoSubUE(const pat::Photon& photon, int pfId, double r1=0.4, double r2=0.00, double threshold=0, double jWidth=0.0, int footprintRemoval = 0, bool excludeCone = false);
