...
// $Id: HiOniaAnalyzer.cc,v 1.26 2013/07/08 15:43:52 tdahms Exp $
//
//
...
//  std::pair< int, float > findGenMCInfo(reco::GenParticleRef genJpsi);
  ...
  */
  int muType; // type of muon (global=1, tracker=2)
...
//    cout << "mapTriggerNameToIntFired_[" << theTriggerNames.at(iTr) << "] " << mapTriggerNameToIntFired_[theTriggerNames.at(iTr)] << endl;
...
//      cout << "iTr+NTRIGGERS " << iTr << " " << NTRIGGERS << " " << iTr+NTRIGGERS << endl;
  ...
  if ( hltConfigInit ) {
    ...
    for (std::map<std::string, unsigned int>::iterator it = mapTriggernameToHLTbit.begin(); it != mapTriggernameToHLTbit.end(); it++) {
      ...
      if (triggerIndex >= n) {
        ...
        std::cout << "[HiOniaAnalyzer::hltReport] --- TriggerName " << it->first << " " << triggerIndex << " " << it->second << " available in config!" << std::endl;
      }
    }
  }
...
//        cout << "    " <<  mapTriggerNameToIntFired_[triggerPathName] << endl;
