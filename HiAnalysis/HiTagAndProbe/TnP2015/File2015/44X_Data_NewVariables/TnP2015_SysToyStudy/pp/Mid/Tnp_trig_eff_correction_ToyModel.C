...
void Tnp_trig_eff_correction_ToyModel(string dataFile="Jpsi_Regit_RD_2014_Trg2_Eff.root", string mcFile="Jpsi_Regit_MC_2014_Trg2_Eff.root", bool savePlots=true)
{
  ...
  dataFile="tnp_pp_RD_Mid.root";
  mcFile="tnp_pp_MC_Mid.root";
  ...
  leg1->Draw("same");
  lt1->DrawLatex(0.16,0.9,"pp  #sqrt{s} = 2.76");
  ...
  for(int i = 0; i < nBin; i++){
    ...
    fEffRDPt[i]->SetParameter(1, errRDh[i]);
    fEffRDPt[i]->SetParameter(2, errRDl[i]);
  }
  ...
  */
  ...
  if (savePlots) {
    if(etaRegion == 1){
      ...
      c1->SaveAs("tnp_trig_eff_mid_pp.png");
      c1->SaveAs("tnp_trig_eff_mid_pp.pdf");
    }
    if(etaRegion == 2){
      c1->cd();
      lt1->DrawLatex(0.16,0.84,"1.6 < |#eta^{#mu}| < 2.4");
      c1->SaveAs("tnp_trig_eff_forward_pp.png");
      c1->SaveAs("tnp_trig_eff_forward_pp.pdf");
    }
  }
